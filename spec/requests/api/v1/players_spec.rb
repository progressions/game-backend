...
RSpec.describe 'Api::V1::Players', type: :request do
  ...
  describe 'POST /api/v1/players' do
    it 'creates a new player with a current room' do
      ...
      json = JSON.parse(response.body)
      expect(json['current_room']['id']).to eq(room1.id)
    ...
    it 'returns 422 if room and game do not match' do
      other_game = Game.create!(title: "Other game", user: user, theme: theme)
  ...
  describe 'POST /api/v1/players/:id/move' do
    it 'moves the player to a connected room' do
      post "/api/v1/players/#{player.id}/move", params: { connection_id: connection.id, room_id: room2.id, }, headers: { 'Authorization' => "Bearer #{token}" }
      expect(response).to have_http_status(:ok)
      json = JSON.parse(response.body)
      expect(json['message']).to eq('Moved through Mossy Trail to Mossy Glade.')
    ...
    it 'returns 422 for ungenerated destination room' do
      ...
      expect(JSON.parse(response.body)['errors']).to include('No valid connection to destination room')
    end
